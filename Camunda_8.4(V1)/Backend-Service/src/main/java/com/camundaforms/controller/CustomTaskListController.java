...
//	List<Map> inputForm = new ArrayList<>();
	...
	System.err.println("this is the componentsNode :"+componentsNode);
	for (JsonNode ref : componentsNode) {
		...
		String defaultValue= (String)ref.path("defaultValue").asText();
		String description= (String)ref.path("description").asText();
		...
		for (Map ref1 : outputForm) {
			...
			mpfin.put("defaultValue", defaultValue);
			mpfin.put("description", description);
			formList.add(mpfin);
		}
	}
...
//	finalMap.put("Forms", componentsNode);
	return finalMap;
}
